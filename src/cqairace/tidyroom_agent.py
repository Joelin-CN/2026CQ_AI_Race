...
class TidyroomAgent(VLMAgent):
    ...
    _MAX_BASE_Z = 110.0      # 候选基座高度上限（place_location Z）。30→110：
                              # R30-32 凉鞋初始生成在茶几面(z=45)曾被 30 误杀——
                              # 生成器"初始全错"会把物品放错误家具上（桌面 45/
                              # 餐桌 79/沙发 99 全合法）；背景构件最低 129
                              # （R12-58 挂墙盒），110 居中分界（99|129）
